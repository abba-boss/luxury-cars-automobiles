...
VEHICLE_DATA=$(curl -s http://localhost:3002/api/vehicles | jq '.data[0]')
...
echo "   - Testing: http://localhost:3002/uploads/$FIRST_IMAGE"
HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3002/uploads/$FIRST_IMAGE)
echo "   - Status: $HTTP_STATUS $([ "$HTTP_STATUS" = "200" ] && echo "✅ SUCCESS" || echo "❌ FAILED")"
echo

# Test 3: Placeholder Image
echo "3. Testing Placeholder Image:"
PLACEHOLDER_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3002/uploads/placeholder-car.svg)
...
CORS_TEST=$(curl -s -H "Origin: http://localhost:8081" -I http://localhost:3002/api/vehicles | grep -i "access-control-allow-origin")
...
echo "   - Sample frontend URL: http://localhost:3002/uploads/$FIRST_IMAGE"
FRONTEND_URL_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:3002/uploads/$FIRST_IMAGE")
